...
var author$project$Main$initialModel = F2(
	function (key, url) {
		return {
			...
			currentKey: '',
			currentKnowledge: elm$core$Dict$empty,
			homescreen: true,
			key: key,
...
var author$project$Main$init = F3(
	function (flags, url, key) {
		return _Utils_Tuple2(
			A2(author$project$Main$initialModel, key, url),
...
var author$project$Main$after = F2(
	function (time, msg) {
		return A2(
			elm$core$Task$perform,
...
var author$project$Main$decreaseKnowledge = function (knowledge) {
	if (knowledge.$ === 'Nothing') {
...
var author$project$Main$increaseKnowledge = function (knowledge) {
	if (knowledge.$ === 'Nothing') {
...
var author$project$Main$nextKey = function (model) {
	var _n0 = function () {
		var _n1 = elm$core$List$head(model.keysToTest);
		if (_n1.$ === 'Just') {
			var key = _n1.a;
			return _Utils_Tuple2(
				...
				A2(
					...
					model.keysToTest,
					elm$core$List$tail(model.keysToTest)));
		} else {
			return _Utils_Tuple2(elm$core$Maybe$Nothing, model.keysToTest);
		}
	}();
	var nextNote = _n0.a;
	var keysRemaining = _n0.b;
	if (nextNote.$ === 'Nothing') {
		return _Utils_update(
			model,
			{winner: true});
	} else {
		var newKey = nextNote.a;
		return _Utils_update(
			model,
			{
				currentKey: newKey,
				currentKnowledge: A3(elm$core$Dict$insert, newKey, 0, model.currentKnowledge),
				homescreen: false,
				keysTesting: A2(elm$core$List$cons, newKey, model.keysTesting),
...
var author$project$Main$weighKnowledge = function (knowledge) {
	return (!knowledge) ? 2 : (1 / knowledge);
};
...
var author$project$Main$randomNoteGenerator = function (model) {
	var knowledgeList = elm$core$Dict$toList(model.currentKnowledge);
	var weightedKnowledge = A2(
		elm$core$List$map,
		function (_n0) {
			var note = _n0.a;
			var knowledge = _n0.b;
			return _Utils_Tuple2(
				author$project$Main$weighKnowledge(knowledge),
				note);
		},
		knowledgeList);
	var tail = A2(
		...
		elm$core$List$tail(weightedKnowledge));
	var head = A2(
		elm$core$Maybe$withDefault,
		_Utils_Tuple2(1, 'C'),
		elm$core$List$head(weightedKnowledge));
					...
					return _Utils_eq(note, model.currentKey) ? _Utils_Tuple2(
						_Utils_update(
							model,
							{
								answer: elm$core$Maybe$Just(
									_Utils_Tuple2(note, true)),
								currentKnowledge: A3(elm$core$Dict$update, note, author$project$Main$increaseKnowledge, model.currentKnowledge),
						...
						elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									author$project$Main$playSound(note),
									A2(author$project$Main$after, 1000, author$project$Main$NextLevel)
								]))) : _Utils_Tuple2(
						_Utils_update(
							model,
							{
								answer: elm$core$Maybe$Just(
									_Utils_Tuple2(note, false)),
								currentKnowledge: A3(elm$core$Dict$update, note, author$project$Main$decreaseKnowledge, model.currentKnowledge)
							}),
						author$project$Main$playSound(note));
				case 'NextSound':
					var _n2 = A2(
						elm$random$Random$step,
						author$project$Main$randomNoteGenerator(model),
						model.seed);
					var nextNote = _n2.a;
					var nextSeed = _n2.b;
					return _Utils_Tuple2(
					...
					if (A2(author$project$Main$readyForNextLevel, model.keysTesting, model.currentKnowledge)) {
						var newModel = author$project$Main$nextKey(
							_Utils_update(
								model,
								{answer: elm$core$Maybe$Nothing}));
						var $temp$msg = author$project$Main$NextSound,
							$temp$model = newModel;
						msg = $temp$msg;
						model = $temp$model;
						continue update;
...
var author$project$Main$viewChoice = F4(
	function (width, prevAnswer, correctNote, note) {
		var buttonClass = function () {
			if (prevAnswer.$ === 'Nothing') {
				return 'button choice';
			} else {
				if (prevAnswer.a.b) {
					var _n1 = prevAnswer.a;
					var answeredNote = _n1.a;
					return _Utils_eq(answeredNote, note) ? 'button choice correct' : 'button choice';
				} else {
					var _n2 = prevAnswer.a;
					var answeredNote = _n2.a;
					return _Utils_eq(answeredNote, note) ? 'button choice incorrect' : 'button choice';
				}
			}
		}();
		return A2(
			...
			_List_fromArray(
				[
					...
					A2(elm$html$Html$Attributes$style, 'flex-basis', width),
					elm$html$Html$Events$onClick(
					author$project$Main$Answer(note))
				]),
			_List_fromArray(
				[
...
var author$project$Main$widthFromChoiceCount = function (choiceCount) {
	return (choiceCount <= 4) ? '50%' : '33%';
};
var author$project$Main$viewChoices = function (model) {
	var choices = model.keysTesting;
	var widthPerChoice = author$project$Main$widthFromChoiceCount(
		elm$core$List$length(choices));
	return A2(
		elm$html$Html$div,
		_List_fromArray(
			[
				elm$html$Html$Attributes$class('row choices')
			]),
		A2(
			elm$core$List$map,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('title row grow-2')
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								elm$html$Html$text('Pitch Perfect')
							]))
					])),
				A2(
				elm$html$Html$div,
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('winner row grow')
					]),
				_List_fromArray(
					[
						elm$html$Html$text(
						model.winner ? 'PERFFECT PITCH!' : '')
					])),
				A2(
				elm$html$Html$div,
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('controls row grow')
					]),
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('status row grow')
					]),
				_List_fromArray(
					[
						A2(
...
var author$project$Main$viewGame = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				elm$html$Html$Attributes$class('column game')
			]),
		_List_fromArray(
			[
				author$project$Main$viewHeader(model),
				author$project$Main$viewChoices(model)
			]));
};
...
var author$project$Main$viewHomescreen = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				elm$html$Html$Attributes$class('column center homescreen')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('start-button button'),
...
var author$project$Main$view = function (model) {
	return {
		body: model.homescreen ? _List_fromArray(
			[
				author$project$Main$viewHomescreen(model)
			]) : _List_fromArray(
			[
